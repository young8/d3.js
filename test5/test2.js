...
function update(data, name, cx, cy) {
  var circles = svg.selectAll("#" + name).data(d3.range((data/10)%300))
  var r=2;
...
d3.csv("PopPerDoc.csv", type, function(error, data) {
  ...
  for (var i = 0; i < alen; i++) {
    update(array[i],"circle"+i,25,i*100+20);
    // var tmp = array[i];
    // var rcount = 0;
    // var rsum = 0;
    // while (tmp > 0) {
  ...
  svg.selectAll(".keys")
    ...
    .on("click", function(d) {
      ...
      var alen = array.length;
      for (var i = 0; i < alen; i++) {
        update(array[i],"circle"+i,25,i*100);
        // var tmp = array[i];
        // var count = 0;
        // var rsum = 0;
        // while (tmp > 0) {
